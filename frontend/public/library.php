...
<div class="header">
  <div class="hdr-top">
    ...
    <div class="hdr-actions">
      <button class="hdr-btn hdr-btn-cr">+ Novo Documento</button>
      <a href="dashboard.php" class="ava-sm"><?php echo $userInitials; ?></a>
    </div>
  </div>
  <div class="hdr-filters" id="hdr-filters">
    ...
    <button class="qf" data-filter="free">Grátis</button>
    <button class="qf" data-filter="paid">Pagos</button>
...
function applyFilters() {
  filteredDocs = allDocs.filter(doc => {
    ...
    if (filters.search && !doc.title.toLowerCase().includes(filters.search.toLowerCase()) && !doc.authors.toLowerCase().includes(filters.search.toLowerCase())) return false;
  ...
  el.innerHTML = filteredDocs.map(doc => `
    <div class="doc-card">
      <div class="dc-thumb">
        ...
        ${+doc.price_kz === 0 ? '<div class="dc-badge-free">Grátis</div>' : ''}
        <div class="dc-badge-cat">${doc.category_id || doc.cat}</div>
      </div>
      <div class="dc-body">
        <div style="font-weight:700;font-size:15px; margin-bottom: 5px;">${doc.title}</div>
        <div style="font-size:12px;color:var(--tx-l);">${doc.course || ''} | ${doc.file_size}</div>
        <div style="font-size:12px;color:var(--tx-l);">${doc.year || ''}</div>
      </div>
...
function openDetail(id) {
  window.location.href = `document-detail.php?id=${encodeURIComponent(id)}`;
}

function openReader(id) {
  window.location.href = `document-detail.php?id=${encodeURIComponent(id)}&read=1`;
}
